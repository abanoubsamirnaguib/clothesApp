...
const setCategory = categorySlug => {
  if (!isDragging.value && (route.query.category || '') !== categorySlug) {
    router.push({ query: { ...route.query, category: categorySlug || undefined } });
  }
};
    ...
    <div class="slider-wrapper">
      <div ref="cardsSlider" class="cards-slider" @scroll="updateButtonVisibility">
        ...
        <div
          ...
          :class="['card text-black transition cat-button-bezel', route.query.category === category.slug ? 'selected' : getCategoryClass(i)]">
          <NuxtImg
            :alt="category.name"
            v-if="category.image"
            loading="lazy"
          ...
          <div
            v-else
            aria-hidden="true"
            class="w-[38px] h-[38px] rounded-full border border-transparent dark:bg-black/15 bg-white/30" />
