...
<template>
  <div
    ...
    <Transition name="fade" mode="out-in">
      <div v-if="cart.length" class="flex w-full h-full max-md:flex-col max-md:max-h-[calc(100vh-92px)] max-md:overflow-auto">
        <div class="w-[calc(100vw-24px)] sm:w-full md:w-80 relative">
          <div class="md:absolute h-full w-full overflow-auto">
            <div
              ...
              <NuxtImg :src="product.image" class="w-24 h-28 object-cover shadow-md rounded-2xl" />
              ...
              <div
                ...
                <div class="dark:bg-white/10 bg-white/50 dark:hover:bg-white/30 hover:bg-white/100 transition-all rounded-full p-0.5 w-5 h-5 flex items-center justify-center">
                  <UIcon size="14" name="i-iconamoon-sign-plus" class="text-black dark:text-white cursor-pointer" @click="increment(product.product_id, product.size)" />
                </div>
                ...
                <div class="dark:bg-white/10 bg-white/50 dark:hover:bg-white/30 hover:bg-white/100 transition-all rounded-full p-0.5 w-5 h-5 flex items-center justify-center">
                  <UIcon
                    ...
                    @click="decrement(product.product_id, product.size)" />
                </div>
              </div>
            </div>
          </div>
