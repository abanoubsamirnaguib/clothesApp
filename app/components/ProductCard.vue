      ...
      <div class="cursor-pointer transition ease-[ease] duration-300">
        <div class="relative pb-[133%] dark:shadow-[0_8px_24px_rgba(0,0,0,.5)] rounded-2xl overflow-hidden">
          <NuxtImg
            ...
            loading="lazy"
            :title="product.name"
            :src="product.images?.[1] || product.galleryImages?.nodes?.[0]?.sourceUrl || product.image?.sourceUrl"
        ...
        <div class="grid gap-0.5 pt-3 pb-4 px-1.5 text-sm font-semibold">
          ...
          <div v-if="product.style || product.allPaStyle?.nodes?.[0]?.name" class="font-normal text-[#5f5f5f] dark:text-[#a3a3a3]">
            {{ product.style || product.allPaStyle.nodes[0].name }}
          </div>
        </div>
      </div>
    </NuxtLink>
